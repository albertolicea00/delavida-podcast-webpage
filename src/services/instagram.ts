	...
	for (let attempt = 0; attempt <= retries; attempt++) {
		try {
			...
			if (response.status === 429) {
				console.warn(
					`[Instagram] Rate limited for @${username} (attempt ${attempt + 1})`
				);
				continue;
			}
			...
			if (attempt === retries) {
				console.warn(
					`[Instagram] Failed to fetch @${username} after ${retries + 1} attempts`
				);
			}
		}
	}
...
export async function fetchAllInstagramData(
	...
	for (const { id, instagramHandle } of characters) {
		...
		if (data) {
			console.log(
				`[Instagram] Got data for @${instagramHandle}: ${data.posts.length} posts`
			);
			dataCache[id] = data;
		} else {
