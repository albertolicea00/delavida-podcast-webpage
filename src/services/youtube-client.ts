...
interface ClientYouTubeData extends Pick<
	YouTubeData,
	"channel" | "latestVideos" | "popularVideos"
> {
	timestamp: number;
}
...
async function fetchText(url: string): Promise<string | null> {
	const candidates = [
		url,
		...RSS_PROXIES.map((proxy) => `${proxy}${encodeURIComponent(url)}`),
	];
...
> | null> {
	...
	const videos = parseRSSFeed(xml).slice(0, poolSize).map(mapRSSVideo);
...
> | null> {
	...
	const channelResponse =
		await fetchJson<YouTubeApiChannelResponse>(channelUrl);
	...
	const playlistResponse =
		await fetchJson<YouTubeApiPlaylistItemsResponse>(playlistUrl);
	...
	const rawVideos = videosResponse.items
		.map((item) => {
			...
			const { thumbnail, thumbnailHigh } = getBestThumbnail(
		...
		.filter(
			(item): item is { video: YouTubeVideo; rawViewCount: number } => !!item
		)
		.sort((a, b) => {
...
function readCachedYouTubeData(
	cacheKey: string
): Pick<YouTubeData, "channel" | "latestVideos" | "popularVideos"> | null {
	try {
...
export async function fetchAllYouTubeDataClient(
	...
	"channel" | "latestVideos" | "popularVideos"
> | null> {
	const { apiKey = "", channelId, maxLatest = 12, maxPopular = 6 } = options;
	...
	if (!data && apiKey) {
		data = await fetchYouTubeDataFromApi(
			channelId,
			apiKey,
			maxLatest,
			maxPopular
