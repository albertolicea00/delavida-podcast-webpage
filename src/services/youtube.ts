...
async function scrapeChannelStats(handle: string): Promise<{
	...
	try {
		...
		const videoCountMatch = html.match(
			/"videosCountText":\s*\{"runs":\s*\[\{"text":\s*"([^"]+)"\}/
		);
		const descMatch = html.match(/"description":\s*"([^"]*?)(?:"|\\)/);
		const thumbMatch = html.match(
			/"avatar":\s*\{"thumbnails":\s*\[.*?\{"url":\s*"([^"]+)"/
		);
...
async function getChannelIdFromHandle(handle: string): Promise<string | null> {
	try {
		...
		const channelIdMatch =
			html.match(/"browseId":\s*"(UC[^"]+)"/) ||
			html.match(/"channelId":\s*"(UC[^"]+)"/);
...
export async function getChannelInfo(): Promise<YouTubeChannelInfo | null> {
	const handle =
		import.meta.env.YOUTUBE_CHANNEL_HANDLE || "laslocurasdemiguelin4300";
...
export async function fetchAllYouTubeData(): Promise<YouTubeData> {
	...
	if (!channel) {
		console.warn(
			"[YouTube] Could not fetch channel info. Using fallback data."
		);
